function [Q,R] = house_qr_lp(A,flag,fp)
...
H = @(u,x) chop(x - chop(u*(hgemm(u',x,fp))));
[m,n] = size(A);
n1 = n; m1 = m;
% if flag == 1
%     if m>n
...
nx = max(abs(x));
x1 = chop(x/nx);
nu = chop(nx*chop(sqrt(ip_chop(x1,x1, fp))));
if nu ~= 0
    u = chop(x/nu);
